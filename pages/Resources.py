import streamlit as st

st.markdown("""
### 🤓 Helpful Docs and Resources

- 🇨🇿 [App Repository](https://github.com/pandyandy/demo-app)
            
- 👋 [Hello, World! Data App](https://75299519.hub.north-europe.azure.keboola.com)
            
- 🐙 [Demo Data Apps in Keboola](https://demo.keboola.com/app/data-apps)    
            
- 📄 [Keboola Data Apps Docs](https://help.keboola.com/components/data-apps/)
            
- 📄 [Streamlit Docs](https://docs.streamlit.io/)
            
- 🐍 [SAPI Python Client](https://github.com/keboola/sapi-python-client)
            
- 🐍 [Keboola Streamlit](https://pypi.org/project/keboola-streamlit/)            
""")